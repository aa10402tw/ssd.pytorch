...
class mySSD(nn.Module):
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        # add more boxes !
        self.cfg['steps'] = [2, 8, 16, 32, 64, 100, 300]
        self.cfg['min_sizes'] =  [15, 30, 60, 111, 162, 213, 264]
        self.cfg['max_sizes'] =  [30, 60, 111, 162, 213, 264, 315]
        self.cfg['feature_maps'] = [150, 38, 19, 10, 5, 3, 1]
        self.cfg['aspect_ratios'] = [[2], [2], [2, 3], [2, 3], [2, 3], [2], [2]]
        ...
        # add more boxes !
        self.loc = nn.ModuleList([nn.Conv2d(128, 16, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))] + head[0])
        self.conf = nn.ModuleList([nn.Conv2d(128, 84, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))] + head[1])
        # print(self.loc)
        # print(self.conf)
    ...
    def forward(self, x):
        ...
        # add more boxes !
        s_down = self.Fusion_Up_38(sources[0])
        s_left = self.Fusion_Left_75(x_75)
        s = s_down + s_left
        s_down = self.Fusion_Up_75(s)
        s_left = self.Fusion_Left_150(x_150)
        s = s_down + s_left
        sources.insert(0, s)
        del x_75
        del x_150
...
def build_ssd300_fpn150(phase, size=300, num_classes=21):
    if phase != "test" and phase != "train":
        print("ERROR: Phase: " + phase + " not recognized")
        return
    if size != 300:
...
if __name__ == '__main__':
    ...
    net = build_ssd300_fpn150('test', size=300, num_classes=21)
